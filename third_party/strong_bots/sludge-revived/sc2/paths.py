...
def latest_executeble(versions_dir):
    ...
    for p in versions_dir.iterdir():
        if not (p.is_dir() and p.name.startswith("Base")):
            continue
        executable = p / BINPATH[PF]
        if executable.exists():
            candidates.append((int(p.name[4:]), p))
    if not candidates:
        raise FileNotFoundError(f"No SC2 executable matching {BINPATH[PF]} found under {versions_dir}")
